 ...
 */
class HybridAuth_Logger
{
    function __construct()
    {
        // if debug mode is set to true, then check for the writable log file
        if ( HybridAuth_Auth::$config["debug_mode"] ){
            if ( ! file_exists( HybridAuth_Auth::$config["debug_file"] ) ){
                throw new Exception( "'debug_mode' is set to 'true', but no log file path 'debug_file' given.", 1 );
            }

            if ( ! is_writable( HybridAuth_Auth::$config["debug_file"] ) ){
    ...
    public static function debug( $message, $object = NULL )
    {
        if( HybridAuth_Auth::$config["debug_mode"] ){
            ...
            file_put_contents(
                HybridAuth_Auth::$config["debug_file"],
